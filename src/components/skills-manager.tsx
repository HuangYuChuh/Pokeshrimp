...
export function SkillsManager({ open, onClose }: SkillsManagerProps) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[80vh] w-[520px] flex-col overflow-hidden rounded-2xl border border-border bg-card">
        {/* Header */}
        <div className="flex items-center justify-between px-6 pb-4 pt-5">
          <div>
            ...
            <p className="mt-1 text-[12px] text-muted-foreground">
              Manage installed .skill.md files
            </p>
          </div>
        ...
        {toast && (
          <div
            className={cn(
              "mx-6 mb-4 rounded-lg px-3 py-2 text-[13px] font-medium",
              toast.isError
      ...
      {expanded && (
        <div className="border-t border-border px-3 py-3">
          <div className="space-y-3">
            ...
            {/* Fallback for skills with no details */}
            {skill.requiredTools.length === 0 &&
              skill.inputParams.length === 0 &&
              skill.outputs.length === 0 && (
