# Pokeshrimp — Development Rules

> This file mirrors `CLAUDE.md` for Codex compatibility.
> `CLAUDE.md` is the authoritative source of truth for these rules.
> Every agent and developer MUST follow these rules. No exceptions.
...
```
Write code + tests → Review (simplicity, necessity, surgical) → fix all issues → verify all checks pass → commit
```
...
- Framework: **Vitest** with `@/` path alias
- Location: `{module}/__tests__/{name}.test.ts`
- Coverage scope: `src/core/**/*.ts` only
- Coverage threshold: 30% statements (CI gate)
- Must test: core/ logic, tool `call()`, middleware behavior, CLI args
- Must NOT test: UI rendering, API routes, re-export layers, third-party libs
- Mock strategy: only mock what you can't control (network, external APIs)
- Tests ship with the feature — not as an afterthought
